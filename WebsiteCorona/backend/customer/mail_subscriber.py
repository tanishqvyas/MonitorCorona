...
def info():
	...
	server.ehlo()
	server.starttls()
	server.login('fansforevah@gmail.com', 'lakshmiistrash')

	# Getting all the coustomers
	mail_list = []
	entries = list(Customer.objects.all().only("email","country"))
	mail_list = entries.copy()
	...
	for mailid in mail_list:
		...
		country_data = Country.objects.values().filter(country_name=mailid.country)[0]
		# Message_str = "The current scenario of "+ str(mailid.country) + " is, Confirmed Cases : " + str(country_data.confirmed_cases) + ", Total patients recovered : "+ str(country_data.total_recovered)+ ", Total number of deaths : "+str(country_data.total_deaths) 
		# Message_str = "The current scenario of "+ str(mailid.country) + " is, Confirmed Cases : " + country_data["confirmed_cases"] + ", Total patients recovered : "+ str(country_data["total_recovered"] )+ ", Total number of deaths : "+str(country_data["total_deaths"]) 
		
		Message_str = "The current standings of "+ str(mailid.country) + "\nConfirmed Cases : " + str(country_data["confirmed_cases"]) + "\nTotal Patients Recovered : " + str(country_data["total_recovered"]) + "\nTotal Death Toll : " + str(country_data["total_deaths"])

		fromx = 'fansforevah@gmail.com'
		msg = MIMEText(Message_str)
		msg['Subject'] = 'Corona Cases Updates'
		msg['From'] = fromx
		msg['To'] = mailid.email

		server.sendmail(fromx, mailid.email, msg.as_string())
